...
function checkSlots(
  ...
  let remainingPerkType: number | undefined;
  let remainingGearSetRequirements = 0;
  const remainingGearSetTypes: number[] = [];
  ...
  for (let [key, value] of requirements) {
    ...
    if (armorPerkValues.has(key)) {
      ...
      if (remainingPerkType === undefined) remainingPerkType = key;
    } else {
      remainingGearSetRequirements += remaining;
      for (let i = 0; i < remaining; i++) remainingGearSetTypes.push(key);
    }
  }

  remainingGearSetRequirements = Math.max(0, remainingGearSetRequirements - selectableGearSetItems);
  const totalRemainingRequirements = remainingPerkRequirements + remainingGearSetRequirements;

  if (totalRemainingRequirements > 1) return { valid: false };
  if (totalRemainingRequirements == 0)
    return { valid: true, requiredClassItemType: ArmorPerkOrSlot.Any };

  if (remainingPerkRequirements === 1 && remainingPerkType !== undefined) {
    return {
      valid: availableClassItemTypes.has(remainingPerkType as ArmorPerkOrSlot),
      requiredClassItemType: remainingPerkType,
    };
  }

  const requiredClassItemType = [...new Set(remainingGearSetTypes)].find((key) =>
    availableClassItemTypes.has(key as ArmorPerkOrSlot)
  );
  if (requiredClassItemType == null) {
    return { valid: false, requiredClassItemType: ArmorPerkOrSlot.Any };
  }

  return {
    valid: true,
    requiredClassItemType,
  };
}
...
addEventListener("message", async ({ data }) => {
  ...
  let helmets = items
    ...
    .filter((k) => {
      return (
        config.useEventArmor !== EventArmorType.FestivalOfTheLost ||
        [
