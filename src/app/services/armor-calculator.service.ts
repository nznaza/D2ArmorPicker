...
export class ArmorCalculatorService implements OnDestroy {
  ...
  async calculateArmorSetResults(
    ...
    try {
      ...
      const updateResultsStart = ArmorCalculatorService.updateResultsStart;
      ...
      this.selectedExotics = ArmorCalculatorService.selectedExotics;
      this.results = [];
      ...
      this.inventoryArmorItems = inventoryArmorItems;
      this.permutatorArmorItems = permutatorArmorItems;
      ...
      const emitResults = (sourceBuilds: IPermutatorArmorSet[], perThreadTiers: number[][]) => {
        ...
        this._armorResults.next({
          results: endResults,
          savedResults: endResults.length,
          totalPermutations: this.totalPermutationCount,
      ...
      for (let n = 0; n < nthreads; n++) {
        this.workers[n] = ArmorCalculatorService.workers[n] = new Worker(
        ...
        this.workers[n].onmessage = async (ev) => {
          const data = ev.data;
          ArmorCalculatorService.threadCalculationDoneArr[n] = data.checkedCalculations;
          ArmorCalculatorService.threadCalculationAmountArr[n] = data.estimatedCalculations;
          ArmorCalculatorService.threadCalculationReachableTiers[n] =
            data.reachableTiers || data.runtime?.maximumPossibleTiers || [0, 0, 0, 0, 0, 0];
          ...
          if (
            !ArmorCalculatorService.emittedPossibleCombinations &&
            ArmorCalculatorService.threadCalculationAmountArr.slice(0, nthreads).every((val) => val > 0)
          ) {
            ArmorCalculatorService.emittedPossibleCombinations = true;
            this._totalPossibleCombinations.next(sumTotal);
          }
          ...
          if (
            ArmorCalculatorService.threadCalculationDoneArr.slice(0, nthreads).every((val) => val > 0)
          ) {
            const newProgress = (sumDone / sumTotal) * 100;
          ...
          if (data.previewResults) {
            ...
            if (!emittedPreview && threadFrontier.every((f) => f != null)) {
              ...
              emitResults(previewBuilds, ArmorCalculatorService.threadCalculationReachableTiers);
            }
            return;
          }
